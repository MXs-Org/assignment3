...
def inject_payload(injection_obj, payload):
  ...
  if result: 
    print("Exploitable with payload: " + payload)
...
def iterate_payloads(injection_obj_lst):
  ...
  for injection_obj in injection_obj_lst:
    method, link, params, cookie = injection_obj
    for param in params:
      payloads = open('payloads/command_injection_payloads_test.txt').readlines()
...
if __name__ == '__main__':
  injection_obj = ['POST','http://localhost:8888/commandinjection/commandinjection.php', ['host'], '']
